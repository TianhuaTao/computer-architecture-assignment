...
Instruction::Operand Instruction::Operand::Register(long long id) {
    return Instruction::Operand{true, id};
}

Instruction::Operand Instruction::Operand::Immediate(long long value) {
...
std::vector<Instruction> Instruction::getInstructionQueue(const std::string& filename) {
    ...
    while (inStream) {
        ...
        if (split[0] == "LD") {
            ...
            instruction.operands[0] = Operand::Register(std::stoll(split[1]));

            long long im = std::stoll(split[2], nullptr, 16);
            ...
            if (split[1][0] != 'R' && split[2][0] == 'R' && split[3][0] != 'R') {
                ...
                instruction.operands[1] = Operand::Register(std::stoll(split[2]));
            ...
            if (split[1][0] == 'R' && split[2][0] == 'R' && split[3][0] == 'R') {
                for (int i = 0; i < 3; ++i) {
                    ...
                    instruction.operands[i] = Operand::Register(std::stoll(split[i+1]));
                }
            } else {
                cerr << "Need a register: " << line << endl;
            }
